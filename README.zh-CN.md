...
打开 `http://localhost:3000`，然后在设置里配置至少一个模型供应商。如果当前没有
供应商，Neo Chat 会弹出连接窗口，可粘贴 `newapi_channel_conn` JSON；导入的密钥
会加密保存在当前浏览器中，随后页面会定位到供应商模型列表，引导你至少启用一个
模型。
...
Compose 会在 `http://localhost:3000` 暴露 Neo Chat，并同时启动不对外暴露端口的
`doc-parser` 文档解析容器。默认解析走本地容器，不需要 MinerU/LlamaParse Key；支持
文本层 PDF、DOCX、XLSX、XLS、PPTX 和常见文本文件，扫描 PDF 暂不包含 OCR。
生产 Docker 部署应设置稳定的 BYOK 值，托管或多实例部署应使用共享存储，并且只有在
代理会剥离伪造转发头时才启用 `TRUST_PROXY_HEADERS`。

如果直接从 GHCR 部署自己的仓库，先在服务器 `.env` 中指定两个镜像：

```bash
NEO_CHAT_IMAGE=ghcr.io/<你的用户名>/<你的仓库>:latest
DOC_PARSER_IMAGE=ghcr.io/<你的用户名>/<你的仓库>-doc-parser:latest
DOCUMENT_PARSE_BACKEND=local
DOCUMENT_PARSE_BASE_URL=http://doc-parser:8000
```

然后执行 `docker compose pull && docker compose up -d --force-recreate`。
